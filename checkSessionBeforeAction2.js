function checkSessionBeforeAction(callback) {
    fetch('check_session_and_package2.php')
        .then(response => response.text()) // Assuming response is plain text
        .then(data => {
            console.log('Server response for package 2:', data); // Debugging line to check server response
            ...
            switch (data.trim()) {
                ...
                case 'valid':
                    ...
                    break;
                default:
                    showNotification("โปรดรอ 3 วิ เพื่อให้ระบบรีหน้าเว็บใหม่  ", 'lak.php');
                    console.error('Unexpected server response:', data);
                    break;
            }
        })
        .catch(error => {
            console.error('Error checking session for package 2:', error);
            showNotification("เกิดข้อผิดพลาดในการเชื่อมต่อกับเซิร์ฟเวอร์ โปรดลองใหม่อีกครั้ง", 'lak.php');
        });
}
