...
if ($result->num_rows === 1) {
    ...
    // Check the expiration status of package 1
    $sql = "SELECT expiration_date FROM user_packages WHERE user_id = ? AND package_id = 1";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        error_log("Prepare failed: " . $conn->error);
        die("Prepare failed: " . $conn->error);
    ...
    $package1_status = 'not_found';
    $current_date = date("Y-m-d");

    if ($row = $result->fetch_assoc()) {
        $expiration_date = $row['expiration_date'];
        error_log("Package ID: 1, Expiration Date: $expiration_date");

        if ($current_date < $expiration_date) {
            $package1_status = 'active';
        } else {
            $package1_status = 'expired';
        }
    }

    if ($package1_status === 'expired') {
        echo "package_1_expired";
    } elseif ($package1_status === 'not_found') {
        echo "no_package_1";
    } else {
        echo "valid";
    }
