...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ตรวจสอบว่าผู้ใช้มี package_id 2 หรือไม่
    $check_package_sql = "SELECT package_id FROM user_packages WHERE user_id = ? AND package_id = 2";
    ...
    // อัปเดตเฉพาะข้อมูลของ package_id = 2
    if ($row && $row['package_id'] == 2) {
        if (isset($_POST['update_expiration'])) {
            // อัปเดตวันหมดอายุ (อัปเดตเฉพาะ package_id = 2)
            $update_expiration_sql = "UPDATE user_packages SET expiration_date = ? WHERE user_id = ? AND package_id = 2";
...
if (isset($_GET['remove_date'])) {
    ...
    // ตรวจสอบว่าผู้ใช้มี package_id 2 ก่อนการลบ
    $check_package_sql = "SELECT package_id FROM user_packages WHERE user_id = ? AND package_id = 2";
    $stmt = $conn->prepare($check_package_sql);
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    ...
    if ($row && $row['package_id'] == 2) {
        // ลบข้อมูลแพ็คเกจของผู้ใช้เฉพาะ package_id = 2
        $remove_record_sql = "DELETE FROM user_packages WHERE user_id = ? AND package_id = 2";
        $stmt = $conn->prepare($remove_record_sql);
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
        $stmt->close();
        header("Location: PackageB.php");
        exit();
    }
}
...
// นับจำนวนข้อมูลทั้งหมดที่มี package_id = 2
$total_sql = "SELECT COUNT(*) as total FROM user_packages WHERE package_id = 2";
...
<body>
    <button class="back-btn" onclick="window.location.href='PackageA.php'">กลับไปที่PackageA</button>
    <button class="go-btn" onclick="window.location.href='PackageC.php'">ไปที่หน้าPackageC</button>
    <br><h1>แพ็คเกจเลขลาก</h1><br><br><br>
    <table>
        <tr>
            <th>ID</th>
            <th>ชื่อผู้ใช้</th>
        ...
        if ($user_packages_result->num_rows > 0) {
            ...
            if ($days_remaining >= 0) {
                ...
                // ปุ่มสำหรับลบแพ็คเกจ
                echo "<td>
                        <a href='PackageB.php?remove_date=" . urlencode($row["user_id"]) . "'><button class='remove-date-btn'>ลบแพ็คเกจ</button></a>
                      </td>";
                echo "</tr>";
            }
        } else {
    ...
    <div class="pagination">
        <?php
        if ($page > 1) {
            echo "<a href='PackageB.php?page=" . ($page - 1) . "'>&laquo; ก่อนหน้า</a>";
        }
        for ($i = 1; $i <= $total_pages; $i++) {
            echo "<a href='PackageB.php?page=" . $i . "' class='" . ($i == $page ? "active" : "") . "'>" . $i . "</a>";
        }
        if ($page < $total_pages) {
            echo "<a href='PackageB.php?page=" . ($page + 1) . "'>ถัดไป &raquo;</a>";
        }
        ?>
    </div>
</body>
