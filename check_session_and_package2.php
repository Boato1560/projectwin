...
if ($result->num_rows === 1) {
    ...
    // Check the expiration status of package 2
    $sql = "SELECT expiration_date FROM user_packages WHERE user_id = ? AND package_id = 2";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        error_log("Prepare failed: " . $conn->error);
        die("Prepare failed: " . $conn->error);
    ...
    $package2_status = 'not_found';
    $current_date = date("Y-m-d");

    if ($row = $result->fetch_assoc()) {
        $expiration_date = $row['expiration_date'];
        error_log("Package ID: 2, Expiration Date: $expiration_date");

        if ($current_date < $expiration_date) {
            $package2_status = 'active';
        } else {
            $package2_status = 'expired';
        }
    }

    if ($package2_status === 'expired') {
        echo "package_2_expired";
    } elseif ($package2_status === 'not_found') {
        echo "no_package_2";
    } else {
        echo "valid";
    }
